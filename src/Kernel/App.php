...
final class App
{
    ...
    public static function boot(string $root = PHPCP_ROOT): self
    {
        ...
        /*
         * ไฟล์สำรองอยู่ใต้ `data` ที่เพิ่งกันไปทั้งก้อน — แต่มันไม่ใช่ของ panel
         *
         * ผู้ดูแลต้องเปิดดูได้จริงว่าไฟล์สำรองมีอยู่ไหม ขนาดเท่าไร และวางไฟล์ที่ได้
         * จากเครื่องอื่นเข้ามาเพื่อนำเข้าได้ · ไม่มีบรรทัดนี้ ตัวจัดการไฟล์จะเปิดไปที่นั่น
         * ไม่ได้เลย แล้ว "ไฟล์สำรองเก็บไว้ที่ไหน" ก็ตอบได้แค่ด้วยการเชื่อหน้าจอ
         *
         * ต้องมา**หลัง** protectAlso() เสมอ — ข้อยกเว้นมีความหมายก็ต่อเมื่อของที่มัน
         * ยกเว้นถูกกันไว้แล้ว
         */
        SelfProtection::allowAlso($config->paths->backups());
        Paths::useBackupsDir($config->paths->backups());
