...
# ---------------------------------------------------------------------------
head_ "3. ต้องฟังพอร์ต 25 จากภายนอกจริง"

# **ไฟล์ตั้งค่าที่ถูกต้องไม่ได้แปลว่าเดมอนทำตาม** — `postfix reload` เปลี่ยนพอร์ตที่ฟัง
# ไม่ได้ ต้อง restart · เจอจริงบนเครื่องจริง: main.cf เขียนว่า `all` แต่ Postfix ยังฟัง
# แค่ 127.0.0.1 อยู่ แปลว่าเมลจากอินเทอร์เน็ตไม่มีทางถึงเครื่องเลย และไม่มีอะไรฟ้อง
if ss -ltnH 2>/dev/null | awk '$4 ~ /:25$/ {print $4}' | grep -qvE '^(127\.|\[::1\])'; then
  ok "Postfix ฟังพอร์ต 25 จากทุกหน้าตัดเน็ตแล้ว"
else
  bad "Postfix ยังฟังแค่ loopback ทั้งที่เปิดเมลแล้ว — เมลจากภายนอกจะไม่ถึงเครื่องนี้"
fi

# ---------------------------------------------------------------------------
head_ "4. ต้องไม่เป็น open relay"
...
# `hostname -i` คืน 127.0.1.1 บน Ubuntu (ตาม /etc/hosts) ซึ่งยัง**เป็น loopback**
# จึงอยู่ใน mynetworks และทดสอบ open relay ไม่ได้ · เอาที่อยู่ scope global จริงมาใช้
OWN_IP="$(ip -4 -o addr show scope global 2>/dev/null | awk '{split($4, a, "/"); print a[1]; exit}')"
[ -n "$OWN_IP" ] || OWN_IP="$(hostname -I 2>/dev/null | awk '{print $1}')"
...
case "$RELAY_RC" in
  ...
  2) bad "ต่อพอร์ต 25 จาก $OWN_IP ไม่ได้ — Postfix ไม่ได้ฟังจากภายนอก เมลจากอินเทอร์เน็ตจะไม่ถึงเครื่องนี้" ;;
  *) bad "**เครื่องนี้เป็น open relay** — ห้ามปล่อยขึ้นเครื่องจริงเด็ดขาด" ;;
esac

# ---------------------------------------------------------------------------
head_ "5. IMAP ต้องล็อกอินได้และเห็นเมล"
...
# ---------------------------------------------------------------------------
head_ "6. ลบกล่องแล้วต้องหายจริง"
