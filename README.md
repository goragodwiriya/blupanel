...
ติดตั้งบน **AWS Lightsail** มีข้อจำกัดเฉพาะที่ต้องจัดการก่อน (ไฟร์วอลล์สองชั้น · พอร์ต 25
ถูกปิดจากฝั่ง AWS · ไม่มี swap มาให้) — ดู [คู่มือ Lightsail](docs/INSTALL-LIGHTSAIL.md)
...
| PHP | เห็นทุกเวอร์ชันที่ apt ของเครื่องนี้มี · ติดตั้ง/ถอดถอนได้จากหน้าเว็บ (งานเบื้องหลังผ่าน systemd) · สถานะการรองรับตัดสินจากวันหมดอายุที่ php.net ประกาศ จึงไม่ต้องแก้โค้ดเมื่อมีเวอร์ชันใหม่ · ตั้ง `memory_limit`, `upload_max_filesize`, `max_execution_time` และค่าที่เปลี่ยนบ่อยอื่น ๆ ได้จากหน้าเว็บ แยกรายบัญชี และแยกของหน้าจัดการเอง |
...
| ผู้ใช้โฮสติ้ง | หนึ่งบัญชี = หนึ่ง uid = หนึ่งบ้าน · โควตาโดเมน/ฐานข้อมูล/อีเมล/พื้นที่ดิสก์ · SFTP แบบ chroot · ส่งอีเมลต้อนรับที่รวมรายละเอียดโฮสต์ทั้งหมด และตั้งรหัสผ่านใหม่ให้พร้อมส่งได้จากหน้ารายชื่อ |
...
| ความปลอดภัย | Security Center · rate limit รายเว็บผ่าน fail2ban · firewall · audit log แบบ hash-chain · ใบรับรองจริงให้หน้าจัดการเองพร้อมต่ออายุอัตโนมัติ |
| เมล | กล่องจดหมายจริงบนเครื่อง (Postfix + Dovecot) · โควตารายกล่อง · ตัวส่งต่อ · เซ็น DKIM ด้วย rspamd · ดูคิวขาออกและอ่านหัวเมลได้โดยไม่มาร์คว่าอ่านแล้ว · ลูกค้าเห็นค่า IMAP/POP3/SMTP และเรกคอร์ด DNS ของโดเมนตัวเองพร้อมคัดลอก |
| ไฟล์ตั้งค่า | แก้ไฟล์ส่วนเสริมของ Apache/nginx/Postfix/Dovecot/BIND9 เองได้จากหน้าเว็บ · ตรวจด้วยตัวตรวจของบริการนั้นก่อนใช้ · ไม่กดยืนยันแล้วคืนค่าเดิมให้เอง |
