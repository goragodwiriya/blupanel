...
// --- พื้นที่ของ panel ต้องปิดสนิท ไม่มีข้อยกเว้น -----------------------------

test('ทุกอย่างใต้พื้นที่ของ panel ต้องปิด รวมทั้งไดเรกทอรีไฟล์สำรองเดิม', static function (): void {
    /*
     * **เคยมีรูตรงนี้** — commit dc4425e เปิด `/var/lib/phpcp/backups` ให้ตัวจัดการ
     * ไฟล์เห็น เพราะไฟล์สำรองไปกองอยู่ในพื้นที่ของ panel · ข้อยกเว้นนั้นต้องพึ่งการ
     * ตรวจ `..` ของตัวเองอย่างเดียวเพื่อไม่ให้กลายเป็นทางเข้าถึง `panel.db`
     *
     * ตั้งแต่ไฟล์สำรองย้ายไปอยู่ `<บ้าน>/backup` ของลูกค้า (PLAN-BACKUP-V2 §4.1)
     * ไม่มีเหตุผลให้เจาะรูอีกแล้ว · เทสต์นี้เฝ้าว่ามันจะไม่ถูกเปิดกลับมา — ถ้ามีของ
     * ที่ต้องให้ผู้ใช้เห็น ให้ย้ายของนั้นออกจากพื้นที่ของ panel ไม่ใช่เจาะรูใหม่
     */
    $data = '/var/lib/phpcp';
    ...
    $blocked = [
        ...
        'ตัวไดเรกทอรีเอง' => $data,
        'ที่พักไฟล์นำเข้า' => $data . '/backups',
        'ไฟล์ในที่พักไฟล์นำเข้า' => $data . '/backups/imported-x.tar.gz',
        'ไต่ออกจากที่พัก' => $data . '/backups/../panel.db',
    ];

    foreach ($blocked as $label => $path) {
        assertTrue(
            SelfProtection::isProtectedPath($path),
            "ต้องกันไว้ ({$label}): {$path}",
        );
    }
});
