...
// --- ข้อยกเว้นเฉพาะทาง: ไฟล์สำรอง -------------------------------------------

test('ไดเรกทอรีไฟล์สำรองเปิดได้ แต่ของอื่นใต้ที่เดียวกันยังปิดสนิท', static function (): void {
    /*
     * `/var/lib/phpcp` ถูกกันทั้งก้อนเพราะ `panel.db` อยู่ในนั้น (hash รหัสผ่านและ
     * session) · แต่ `backups/` ข้างในไม่ใช่ข้อมูลของ panel — ผู้ดูแลต้องเปิดดูได้จริง
     * ว่ามีไฟล์สำรองอยู่ไหม ไม่ใช่เชื่อหน้าจออย่างเดียว
     *
     * **ข้อยกเว้นต้องแคบจริง** — เทสต์นี้คือสิ่งเดียวที่กันไม่ให้มันค่อย ๆ กว้างขึ้น
     * จนกลายเป็นทางเข้าถึง panel.db
     */
    $data = '/var/lib/phpcp';

    SelfProtection::protectAlso($data);
    SelfProtection::allowAlso($data . '/backups');

    // เปิดได้ — ไดเรกทอรีของไฟล์สำรองและของข้างใน
    foreach ([$data . '/backups', $data . '/backups/site-example.com.tar.gz', $data . '/backups/ย่อย/x'] as $allowed) {
        assertTrue(
            !SelfProtection::isProtectedPath($allowed),
            "ต้องเปิดให้เข้าถึงได้: {$allowed}",
        );
    }

    // ยังปิด — ทุกอย่างที่เหลือใต้ที่เดียวกัน
    $blocked = [
        'ฐานข้อมูลของ panel' => $data . '/panel.db',
        'ไฟล์ wal ของฐานข้อมูล' => $data . '/panel.db-wal',
        'ชั้นบนของไฟล์สำรอง' => $data,
        'โฟลเดอร์อื่นที่ชื่อใกล้กัน' => $data . '/backups-old',
        'ไต่ออกจากข้อยกเว้น' => $data . '/backups/../panel.db',
    ];

    foreach ($blocked as $label => $path) {
        assertTrue(
            SelfProtection::isProtectedPath($path),
            "ต้องยังกันไว้ ({$label}): {$path}",
        );
    }
});
