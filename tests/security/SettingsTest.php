...
test('คีย์ที่ไม่อยู่ในรายการต้องถูกปฏิเสธ', static function (): void {
     ...
     */
    /*
     * `panel.php.*` เข้าข่ายเดียวกันอีกชุด — ค่า PHP ของ panel เอง
     *
     * ปลอดภัยที่จะเก็บในตารางเพราะไม่มีตัวไหนถูกอ่านตอนบูต (config.php ยังเป็นเจ้าของ
     * พอร์ต กุญแจ และเลย์เอาต์เหมือนเดิม) และทุกค่าผ่าน `PhpSettings::assertValue()`
     * ที่รับเฉพาะจำนวนเต็มในช่วงที่กำหนดกับชื่อไทม์โซนที่ PHP รู้จักจริง — ไม่มีทางกลาย
     * เป็นข้อความอิสระที่หลุดลงไฟล์ค่าตั้ง
     *
     * แต่ต้องแก้ผ่านฟอร์มตั้งค่าทั่วไปไม่ได้ ด้วยเหตุผลเดียวกับ `security.panel_jail.*`:
     * ค่าพวกนี้ต้องเดินทางคู่กับไฟล์ pool ของ panel และ LimitRequestBody ของ Apache
     * ที่ถูกเขียนจากมัน · เขียนแถวโดยไม่เขียนไฟล์เมื่อไร หน้าจอจะบอกว่าอัปโหลดได้ 512M
     * ทั้งที่ไฟล์บนดิสก์ยังเขียนว่า 32M
     */
    foreach (array_keys($keys) as $key) {
        if (str_starts_with($key, 'security.') || str_starts_with($key, 'panel.php.')) {
            $allowedKeys[] = $key;
