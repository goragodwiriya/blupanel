...
test('ฟอร์มที่เปิดใน Modal ต้องสั่งปิด Modal เมื่อบันทึกสำเร็จ', static function (): void {
    /*
     * **Modal ถูกเปิดด้วยคำสั่งจากเซิร์ฟเวอร์ การเก็บกวาดจึงต้องเป็นคำสั่งจากเซิร์ฟเวอร์ด้วย**
     *
     * ไม่มีคำสั่งอะไรเลย ฟอร์มจะค้างอยู่บนจอทั้งที่บันทึกสำเร็จไปแล้ว · ผู้ใช้เห็นฟอร์มเดิม
     ...
     *
     * **มีสองทางที่ถูกต้อง ไม่ใช่ทางเดียว:**
     *   1. `saved()` — ปิดหน้าต่างไปเลย (ฟอร์มทั่วไป)
     *   2. `revealed()` — **สลับเนื้อในเป็นกล่องรหัสผ่านแทนการปิด** · ห้ามปิดก่อนเปิดใหม่
     *      เพราะ Modal.hide() ตั้งเวลาล้างเนื้อในตัวเองไว้ 150ms ซึ่งมาถึงหลังเนื้อหาใหม่
     *      ถูกใส่ไปแล้ว — หน้าต่างจะเปิดขึ้นมาว่างเปล่าพร้อมรหัสผ่านที่หายไปด้วย
     *
    ...
    foreach (glob(PHPCP_ROOT . '/src/Http/V2/*.php') ?: [] as $file) {
        ...
        // เก็บกวาดได้สามทาง: สั่งปิดตรง ๆ · saved() ที่ใส่คำสั่งปิดให้เอง ·
        // revealed() ที่สลับเนื้อในเป็นกล่องค่าที่ดูได้ครั้งเดียว
        foreach (["'action' => 'close'", '$this->saved(', '$this->revealed('] as $closes) {
            if (str_contains($code, $closes)) {
                continue 2;
            }
        }
    ...
    assertSame(
        [],
        $problems,
        "controller เหล่านี้เปิดฟอร์มใน Modal แต่ไม่เคยเก็บกวาด — ใช้ saved() / revealed() หรือใส่ "
