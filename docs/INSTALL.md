# คู่มือติดตั้ง BluPanel

แผงควบคุมเซิร์ฟเวอร์ที่เขียนด้วย PHP ล้วน ไม่มี Composer ไม่มี npm

> ชื่อ **BluPanel** คือชื่อผลิตภัณฑ์ ส่วน **`phpcp`** คือชื่อที่ปรากฏในเครื่องจริง —
> คำสั่ง (`phpcp`), บัญชีระบบ (`phpcp-web`), บริการ (`phpcp-agentd`) และเส้นทางไฟล์
> (`/etc/phpcp`) ใช้ชื่อนี้ทั้งหมด
...
- สิทธิ์ root
- **RAM 1 GB · พื้นที่ว่าง 10 GB** — ตัวเลขนี้เผื่อของที่ตัวติดตั้งลงให้ครบชุด (MariaDB,
  BIND9, Postfix + Dovecot + rspamd, PHP สองเวอร์ชัน) บวกไฟล์เว็บของลูกค้า ·
  ตัว panel เองกินน้อยกว่านี้มาก แต่เครื่องที่ให้แค่นั้นจะเริ่มติดขัดตอนใช้งานจริง
...
> **บน AWS Lightsail ให้ใช้ [INSTALL-LIGHTSAIL.md](INSTALL-LIGHTSAIL.md) แทน** — มีข้อจำกัด
> เฉพาะที่ต้องจัดการก่อนติดตั้ง ไม่งั้นจะเข้าหน้าจัดการไม่ได้และส่งเมลออกไม่ได้
...
เลือกที่เก็บไฟล์ได้ตอนติดตั้ง:

| ตัวเลือก | ผล |
|---|---|
| `--users-dir=/srv/hosting` | ย้ายบ้านของลูกค้าโฮสติ้งไปพาร์ทิชันอื่น (ค่าเริ่มต้น `/home`) · ตัวติดตั้งตั้ง `0711` ให้เอง |
| `--sites-dir=/path` | ที่เก็บของเลย์เอาต์เดิมก่อน migration 0006 — ไม่ต้องใช้ในการติดตั้งใหม่ |
| `--pointer-root=/mnt/data` | อนุญาตให้ DocumentRoot ชี้เข้าโฟลเดอร์นี้ได้ (Domain Pointer) · ใส่ซ้ำได้หลายครั้ง |
| `--shared-owner` | ข้ามการแยกเจ้าของไฟล์ระหว่างเว็บ — **สำหรับ NTFS/exFAT/FAT เท่านั้น** ซึ่งเก็บ ownership ของยูนิกซ์ไม่ได้ · อย่าใช้บนเครื่องจริง |

ดูรายการทั้งหมดพร้อมคำอธิบายได้จาก `./install.sh --help` ซึ่งอ่านจากหัวไฟล์จริง
จึงไม่มีทางตกหล่นเมื่อมีตัวเลือกใหม่
...
เปลี่ยนเป็นใบจริงได้ที่หน้า **SERVER → ตั้งค่า → ใบรับรองของหน้าจัดการ** — ใส่ชื่อโดเมนที่
มีใบรับรองอยู่แล้ว ระบบจะคัดลอกให้และคอยคัดลอกใบใหม่ทุกครั้งที่ certbot ต่ออายุ

จากบรรทัดคำสั่งก็ได้ ซึ่งเป็นทางที่ใช้ได้แม้ตอนที่เข้าหน้าเว็บไม่ได้แล้ว:
...
3. **เปิดหน้าความปลอดภัย** (`/app/security`) แล้วไล่แก้ตามรายการที่ขึ้น
...
/var/lib/phpcp/          ฐานข้อมูล SQLite ของ panel
/var/log/phpcp/          log ของ panel
/home/<ผู้ใช้>/           บ้านของลูกค้าโฮสติ้งแต่ละราย — ไฟล์เว็บอยู่ที่นี่
/srv/phpcp/mail/         Maildir ของทุกกล่องจดหมาย
/srv/phpcp/sites/        เลย์เอาต์เดิมก่อน migration 0006 — ว่างในการติดตั้งใหม่
/run/phpcp/              socket ของ agent
```

### บ้านของลูกค้าโฮสติ้ง

**หนึ่งบัญชี = หนึ่ง uid = หนึ่งบ้าน** ต่อให้มีหลายเว็บ · บัญชี Linux ใช้ชื่อเดียวกับชื่อผู้ใช้
ใน panel และถูกสร้างตอนสร้างเว็บแรกเท่านั้น

รูปทรงใต้บ้านเลือกได้รายบัญชี ค่าเริ่มต้นคือ **cpanel** เพราะเป็นรูปที่ลูกค้าที่ย้ายมาคาดหวัง:

```
/home/somchai/
  public_html/           ไฟล์เว็บของโดเมนหลัก
  <โดเมนอื่น>/            ไฟล์เว็บของโดเมนที่เพิ่มทีหลัง
  logs/<โดเมน>/          log แยกตามโดเมน
  backup/                ไฟล์สำรองของบัญชีนี้ — ลูกค้าดาวน์โหลดเองได้ผ่าน SFTP
  tmp/                   ใช้ร่วมกันทั้งบัญชี (FPM pool เดียวรับทุกเว็บของเจ้าของคนเดียวกัน)
  .phpcp/<โดเมน>/        ของประจำเว็บที่ไม่ใช่ไฟล์เว็บ เช่นหน้าระงับบริการ
```

อีกแบบคือ `phpcp` ซึ่งซ้อนทุกอย่างของเว็บหนึ่งแห่งไว้ในกล่องเดียว
(`/home/somchai/domains/<โดเมน>/{public,logs}`) — เลือกได้ที่หน้าผู้ใช้หรือ
`PUT /api/v2/users/{id}/layout`

ย้ายบ้านไปพาร์ทิชันอื่นได้ตอนติดตั้งด้วย `--users-dir=` และเปลี่ยนค่าเริ่มต้นของทั้งเครื่อง
ได้ที่คีย์ `sites.users_dir` ใน `config.php`

`/home` ถูกตั้งเป็น `0711` เพื่อไม่ให้ลูกค้าไล่ดูรายชื่อกันได้ แต่ยังเดินเข้าบ้านตัวเองได้
...
```bash
ls -ld /home/<ผู้ใช้>/public_html
# ต้องเป็น <ผู้ใช้>:www-data — ถ้ากลุ่มไม่ใช่ www-data เว็บเซิร์ฟเวอร์จะอ่านไม่ได้
```
...
**ยังไม่ลบให้:** `/var/lib/phpcp` (ฐานข้อมูลของ panel), `/home/*` (ไฟล์เว็บไซต์และ
ไฟล์สำรองของลูกค้าทั้งหมด) และ `/srv/phpcp/mail` (กล่องจดหมาย) — ต้องลบเองถ้าแน่ใจจริง ๆ

บัญชี Linux ของลูกค้าโฮสติ้ง (ชื่อเดียวกับชื่อผู้ใช้ใน panel) และฐานข้อมูล MariaDB
ก็ยังอยู่เช่นกัน
