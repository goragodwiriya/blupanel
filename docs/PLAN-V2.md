...
- ❌ **Mail hosting เต็มรูปแบบ** (รับเมล/mailbox/webmail) — ยืนยันตามเหตุผลเดิมใน ROADMAP: ต้องดูแลชื่อเสียง IP ตลอดเวลา ถ้า SPF/DKIM/DMARC/rDNS ไม่ครบเมลเข้าถังขยะหมด และถ้าตั้ง relay ผิดจนเป็น open relay จะกระทบ*ทุกเว็บไซต์*บนเครื่อง งานชุดนี้ใหญ่พอที่จะเป็นผลิตภัณฑ์แยก · **คงไว้แค่เมลขาออก (Postfix loopback-only) ที่ทำเสร็จแล้ว**
...
- **ยังไม่ยืนยันว่าอีเมลถึงกล่องจดหมายจริง** — `phpcp-alert test` รายงานว่าส่งสำเร็จและ
  Telegram มาถึงแล้ว แต่ยังไม่ได้ตรวจกล่องของ `admin@goragod.com` · Postfix รับเข้าคิว
  สำเร็จ ≠ ปลายทางได้รับ
...
1. **`tests/run.php` ต้องผ่าน 100% ทุกครั้งก่อน commit** — ปัจจุบัน 356/356 ห้ามลดลง
...
```bash
php tests/run.php                  # ทดสอบทั้งหมด — ต้องผ่าน 356/356 เสมอ
...
|---|---|---|
| `php tests/run.php` | ในโปรเซส | ทุกครั้งก่อน commit — ต้อง 356/356 |
